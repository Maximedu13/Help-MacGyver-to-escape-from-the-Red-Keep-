...
from constants import SCREENSIZE_GAME, TILESIZE, MACGYVER_IMG, GARDIAN_IMG, BAG_ITEM,\
NBCASES, NO_ITEM, BAG_TXT, WHITECOLOR, TRANSPARENT, ITEM_SOUND
...
class LoadGame():
    ...
    def __init__(self):
        ...
        self.macgyver_img = pygame.transform.scale(self.macgyver_img, (TILESIZE, TILESIZE))
        self.macgyver = Character("MacGyver", 0, 0, self.macgyver_img, self.maze)

        #GARDIAN LOADING
        self.gardian_img = pygame.image.load(GARDIAN_IMG).convert_alpha()
        self.gardian_img = pygame.transform.scale(self.gardian_img, (TILESIZE, TILESIZE))
        self.gardian = Character("Gardienne", NBCASES*TILESIZE - TILESIZE, \
    ...
    def load_items(self):
        ...
        for item in list_items:
            ...
            if (self.macgyver.position_x == list_items[list_items.index(item)].obj_sprite_x) \
            and (self.macgyver.position_y == list_items[list_items.index(item)].obj_sprite_y):
                #IT MAKES A SOUND
    ...
    def run(self):
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                self.maze.display(self.window)
                self.window.blit\
                (self.macgyver_img, (self.macgyver.position_x, self.macgyver.position_y))
                self.window.blit\
                (self.gardian_img, (self.gardian.position_x, self.gardian.position_y))
